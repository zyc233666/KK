    ...
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            if (i == nx / 2 && j == ny / 2) {
                ...
                double distance = x * y;
                hilbert_kernel.at<float>(i, j) = 1 / (CV_PI *CV_PI* distance);
            }
        }
    }

    cv::Mat delta = Mat::zeros(input_img.rows, 1, CV_32F);
    //hilbert_kernel_r.at<float>(0, 0) = 1;
    int n = delta.rows;
    for (int i = 0; i < nr; i++) {
        if (i == n / 2) {
            delta.at<float>(i, 0) = 1;
        }
    }

    //for (int i = 0; i < nr; i++)
    //{
    //    printf("%d: %f\n", i, hilbert_kernel_r.at<float>(i,0));
    //}
    // 对实部和虚部分别应用Hilbert核
    filter2D(input_img, hilbert_re, -1, delta);
    filter2D(input_img, hilbert_im, -1, hilbert_kernel_r);
